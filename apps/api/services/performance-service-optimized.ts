...
export class OptimizedPerformanceService {
  ...
  /**
   * OPTIMIZED: Get daily performance data with single database query
   */
  private async getDailyPerformance(
    ...
    const dateStrings = dates.map(date => this.formatLocalDateString(date));
    
    // Single optimized query to fetch all quest data for the date range
    const allQuests = await db
      ...
      .where(
        and(
          ...
          // Use ANY for better performance than multiple OR conditions
          sql`${questInstance.date} = ANY(${dateStrings})`
        )
      );

    // Group quests by date for efficient processing
    const questsByDate = new Map<string, Array<{ basePoints: number; completed: boolean }>>();
    allQuests.forEach(quest => {
      if (!questsByDate.has(quest.date)) {
        questsByDate.set(quest.date, []);
      }
      questsByDate.get(quest.date)!.push({
    ...
    const performanceData = dates.map((date) => {
      const dateStr = this.formatLocalDateString(date);
      const quests = questsByDate.get(dateStr) || [];
  ...
  /**
   * OPTIMIZED: Aggregate daily data into batches with single query
   */
  private async aggregateDailyData(
    ...
    // Get all dates in a single array for one query
    const allDates = dateGroups.flat();
    const dateStrings = allDates.map(date => this.formatLocalDateString(date));
    ...
    for (const group of dateGroups) {
      ...
      group.forEach(date => {
        ...
        const dayCompletedPoints = quests.filter(q => q.completed).reduce((sum, q) => sum + q.basePoints, 0);
        const dayTotalPoints = quests.reduce((sum, q) => sum + q.basePoints, 0);
        const dayPercentage = dayTotalPoints > 0 ? (dayCompletedPoints / dayTotalPoints) * 100 : 0;
        
        totalCompletedPoints += dayCompletedPoints;
        totalPossiblePoints += dayTotalPoints;
        totalQuests += quests.length;
        totalCompletedQuests += quests.filter(q => q.completed).length;
        totalPercentage += dayPercentage;
      });

      // Average the daily percentages
      const averagePercentage = Math.round(totalPercentage / group.length);

      aggregatedData.push({
        day: label,
        date: this.formatLocalDateString(startDate),
...
export const optimizedPerformanceService = new OptimizedPerformanceService();