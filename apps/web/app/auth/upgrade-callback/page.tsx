...
export default function UpgradeCallbackPage() {
  ...
  const [status, setStatus] = useState<"loading" | "success" | "error" | "account-exists">("loading");
  ...
  useEffect(() => {
    const handleUpgradeCallback = async () => {
      try {
        ...
        const anonymousUserId = sessionStorage.getItem('anonymousUpgradeUserId');
        ...
        const checkResponse = await fetch("http://localhost:5001/api/check-oauth-account-callback", {
          method: "POST",
          headers: {
        ...
        if (result.accountExists) {
          ...
          setMessage(`This Google account is already registered. You'll need to sign in with that account instead.`);
        ...
        const upgradeResponse = await fetch("http://localhost:5001/api/complete-oauth-upgrade", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
        ...
        if (upgradeResult.success) {
          setStatus("success");
          setMessage("Account upgraded successfully! Your guest progress has been saved permanently.");
      ...
      <div className="max-w-md w-full bg-zinc-800/50 backdrop-blur-sm border border-zinc-700 rounded-lg p-6 text-center">
        {status === "loading" && (
          <>
            ...
            <h2 className="text-xl font-bold text-zinc-100 mb-2">Processing Account Upgrade</h2>
            <p className="text-zinc-400">Verifying your Google account and upgrading your guest progress...</p>
          </>
        )}
        ...
        {status === "success" && (
          <>
            ...
            <h2 className="text-xl font-bold text-zinc-100 mb-2">Upgrade Successful!</h2>
            <p className="text-zinc-400 mb-4">{message}</p>
            <Alert className="bg-green-900/20 border-green-700">
              <AlertDescription className="text-green-300">
                🎉 Your guest progress has been permanently saved to your Google account!
              </AlertDescription>
            </Alert>
          </>
        )}
        ...
        {status === "account-exists" && (
          <>
            ...
            <h2 className="text-xl font-bold text-zinc-100 mb-2">Account Already Exists</h2>
            <p className="text-zinc-400 mb-4">{message}</p>
            ...
            <Alert className="bg-yellow-900/20 border-yellow-700 mb-4">
              <AlertDescription className="text-yellow-300">
                ...
                <br />
                To access that account, please sign in normally instead of upgrading.
              </AlertDescription>
            </Alert>
        ...
        {status === "error" && (
          <>
            ...
            <h2 className="text-xl font-bold text-zinc-100 mb-2">Upgrade Failed</h2>
            <p className="text-zinc-400 mb-4">{message}</p>
            
            <Alert className="bg-red-900/20 border-red-700 mb-4">
              <AlertDescription className="text-red-300">
                Don't worry - your guest progress is still safe. You can try upgrading again.
              </AlertDescription>
            </Alert>
